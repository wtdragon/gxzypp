...
use Area,City,College,Specialty,Province,UserProfile,ProfileField,Ktest,Kresult,Flzhuanye,Student,Zylb;
...
class FlzhuanyeController extends \BaseController {
	...
	public function index()
	{
		...
		//var_dump($userprofile->first_name);
		    if($loggeduser)
			{
			 ...
			 $pre_page = 20;//每页显示页数
		     $flzhuanye = Flzhuanye::paginate($pre_page);
			 
		     return \View::make('backend.zyjs')->with('user',$userprofile)
			                                       ->with('zyjss',$flzhuanye);
			}
			else{
		 	$logged='not login';
