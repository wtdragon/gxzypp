...
use Mschool,UserProfile,Zylb,Specialty,ProfileField,Kmajor,Kcareer,Careersalay,Careervideo,Trend,Careermajors,Student;
	...
	public function index()
	{
		...
		//var_dump($userprofile->first_name);
		    if($loggeduser)
			{
			 ...
			 $pre_page = 20;//每页显示页数
		     $careersalays= Careersalay::paginate($pre_page);
			 $videos= Careervideo::paginate($pre_page);
			 $trend= Trend::paginate($pre_page);
		     return \View::make('backend.careers')->with('user',$userprofile)
		         ->with('careersalays',$careersalays)
			                                      ->with('trend',$trend)
			                                       ->with('videos',$videos);
			}
			else{
...
//var_dump($str2); 
	...
	public function edit($id)
	{
		...
		return \View::make('backend.edit')->with('user',$userprofile)
										  ...
										  ->with('mschool', null)
										  ->with('ktests', null)
										  ->with('careersalay', Careersalay::find($id));
	}
	...
	{
		//
			$careersalay =Careersalay::find($id);
 
 
$careersalay->srsptu=Input::get('srsptu');
$careersalay->gzjygztj=Input::get('gzjygztj');
$careersalay->lngzbh=Input::get('lngzbh');
